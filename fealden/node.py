...
'''--   --------------------------------------------------------------------
    ...
    It stores: a pointer to it's upstream SSNode
               a pointer to the SSNode attached from strand 1
               a pointer to the SSNode attached from strand 2
...
---------------------------------------------------------------------'''
class DSNode(Node):
    DIST_MULTIPLIER = 2 #The constant multiplier for the distance between bps on DS nodes
    ...
    def __init__(self, progenitor, length = -1):
        ...
        self.length           = length
        self.seq              = ''
        self.relLocRecStart   = -1     #set only if this node contains recSeq, is rel to beginning of node
    ...
    '''
        ...
        lead to index2, and from a link, arriving from index1, to index2. In the first case
        ...
        We need to calculate on either side of the link separately. We can count 5 integers
        ...
        is due to the fact that, in calculating the euclidean distance between 1 and 10,
        ...
        between an index and a link (ie. between 1 and Link in our example), and it should
...
class SSNode(Node):
    ...
    '''
    def get_distance(self, link1, link2):
        #this is an SSNode so distance between links is always the length of this node
        return self.length
    ...
    '''
        ...
        and it has a slightly odd feature: In order to calculate the distance between 
        index1 and index2, we must calculate the distances from index1 to a link that will
        lead to index2, and from a link, riving from index1, to index2. In the first case
        we do not wish to count index1 as part of the distance. In the second case we do
        count index2. The reason for this disparity can be seen if one tries to calculate
        the distance between 1 and 10 as follows:
            1-2-3-4-5-6-Link-7-8-9-10
